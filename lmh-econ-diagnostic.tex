...
\paragraph{Instructions:} This is a diagnostic test to help us determine which economics courses at Oxford will be appropriate for you, given your background. 

Depending on the economics courses you have taken or are currently taking, you will be asked to answer a number of relevant questions. Each section below begins with an explanation of which applicants should attempt the questions in that section. For a summary of this information, see the table at the bottom of this page. 

Answer each question to the best of your ability. Some questions are short and others are longer, but even the very longest should take less than an hour. If you get stuck, or encounter a topic or concept with which you are unfamiliar, please note this down in lieu of a full answer and move on to attempt the remaining questions. As a rough guide, expect to spend between 45 and 90 minutes on each section that you are required to complete. 

This document may be updated from time to time, e.g.\ to fix typos. Please click on the link above to ensure you have the most recent version!
...
\begin{center}
  \begin{tabular}[h]{cl}
  \textbf{Section} & \textbf{Attempt if you have taken or are currently taking:}\\
    \hline
    A & \emph{All applicants should complete this section.}\\
    B & Introductory Probability \& Statistics \\
    C & Introductory Econometrics or equivalent\\    
    D & Intermediate Micro or equivalent\\
    E & Advanced Micro or equivalent\\
    F & Any course that includes some Game Theory\\
    G & Intermediate Macro or equivalent\\
    H & Advanced Macro or equivalent\\
...
\textbf{All applicants} should attempt this question. 

\begin{questions}
\question \begin{parts}
  \part Solve the simultaneous equations $y = x^2 + 1$ and $2y - 3x = 4$.
\part Find all solutions of the equation $x^3 - 2x^2 - 9x + 18 = 0$.
\part Simplify $\frac{1}{2}\ln x^4 + 3\ln(2y)$.
\part Evaluate the sum of the infinite series $2 + 1 + 0.5 + 0.25 + 0.125 + \cdots$.
\part Find all stationary points of the function $f(x,y) = x^3 - y^3 - 3x + 12y$, and classify them.
\part Find the maximum and the minimum of the function $f(x,y) = 2x + y^2$ subject to the constraint $x + y = 4$, where $x$ and $y$ are both non-negative real numbers.
\end{parts}

\bigskip

\fullwidth{\section{Introductory Probability \& Statistics}
Attempt these questions if you have taken or are currently taking \textbf{Introductory Probability \& Statistics} or an equivalent course.}
...
\fullwidth{\section{Econometrics}
Attempt these questions if you have taken or are currently taking \textbf{Introductory Econometrics} or another econometrics course, for example \textbf{Applied Econometrics}.}
...
\end{align*}
  \begin{parts}
    ...
    \begin{subparts}
        \subpart A measure of average rainfall in the region where the oranges are grown.
        \subpart Income per capita in the region where the oranges are sold.
    \end{subparts}
  \end{parts}
...
Attempt these questions if you have taken or are currently taking an \textbf{Intermediate Microeconomics} or an \emph{advanced} Introduction to Microeconomics course.}
...
Attempt these questions if you have taken or are currently taking \textbf{Advanced Microeconomics} or an equivalent course.} 
...
\begin{parts}
  ...
  \uplevel{For the rest of this question, assume that the residents agree to provide flood defences through voluntary contributions. Each resident $i$ purchases a quantity $q_i$ of flood defences, taking the contributions of all other residents as given.}
...
\begin{parts}
  ...
  \uplevel{Norma is risk neutral. Her income will be 3 if state A occurs and 7 if state B occurs. Suppose that Arthur and Norma can only write contract of the form ``Arthur will give Norma amount $x$ if and only if state A occurs, and Norma will give Arthur an amount $y$ if an only if state B occurs''.}
  ...
  \uplevel{Now assume that Arthur's utility as a function of his income $m$ is $\ln m$.}
...
Attempt these questions if you have taken or are currently taking a \textbf{Microeconomics} course that includes some game theory, or an introductory \textbf{Game Theory} course.}
...
\begin{parts}
  ...
  \begin{subparts}
    \subpart Each player decides between \emph{Green} and \emph{Red}. The payoffs are as follows: if both choose the same action, each receives 5; otherwise the player who chooses \emph{Green} receives 6 and the player who chose \emph{Red} receives 4.
    \subpart Each player decides between \emph{Blue} and \emph{Yellow}. The payoffs are as follows: if both choose \emph{Blue}, each receives 3; if both choose \emph{Yellow}, each receives 5; otherwise the player who chose \emph{Blue} receives 4 while the player who chose \emph{Yellow} receives 2.
  \end{subparts}
...
\question Consider the following two-player, one-shot, simultaneous-move game:
    ...
    \begin{center}\begin{tikzpicture}
        \path [fill=black!10] (2,2) -- (2,0) -- (0,2);
        \path [fill=black!10] (4,2) -- (4,0) -- (2,2);
        \path [fill=black!10] (2,4) -- (2,2) -- (0,4);
        \path [fill=black!10] (4,4) -- (4,2) -- (2,4);

        \path [fill=black!70] (-1,0) rectangle (-0.5,4);
        \path [fill=black!70] (0,4.5) rectangle (4,5);

        \node[white, rotate=90] at (-0.75, 2) {\small \textbf{Row Player}};
        \node[rotate=90] at (-0.25, 3) {\footnotesize Up};
        \node at (0.5,2.5) {6}; \node at (2.5,2.5) {3};
        \node[rotate=90] at (-0.25, 1) {\footnotesize Down};
        \node at (0.5,0.5) {5}; \node at (2.5,0.5) {2};

        \node[white] at (2, 4.75) {\small \textbf{Column Player}};
        \node at (1, 4.25) {\footnotesize Left};
        \node at (1.5,3.5) {4}; \node at (1.5,1.5) {3};
        \node at (3, 4.25) {\footnotesize Right};
        \node at (3.5,3.5) {5}; \node at (3.5,1.5) {2};
...
\begin{parts}
  \part This game has a unique pure-strategy Nash equilibrium. Identify it and explain your reasoning. 
  \uplevel{Above we assumed that this was a simultaneous-move game. Now suppose that Row player moves first, followed by Column player.}
...
\begin{parts}
  ...
  \uplevel{Suppose that each firm can only choose between two quantities to produce: either $\frac{1}{2}q^M=3$ (half the joint monopoly quantity) or $q^C=4$ (the Cournot quantity). }
...
Attempt these questions if you have taken or are currently taking \textbf{Intermediate Macroeconomics} or an \emph{advanced} Introduction to Macroeconomics course.}
...
\newpage
\fullwidth{\section{Macroeconomic Theory II} Attempt these questions if you are have taken or are currently taking \textbf{Advanced Macroeconomics} or an equivalent course.}
